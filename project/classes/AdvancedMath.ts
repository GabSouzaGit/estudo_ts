...
export function getMMC(...numbers : number[]){
...
export function getPercentualFrom(...n : number[]) : Percentuals {
 ...
 */
export function isAP(sequence :  number[]){
 ...
 */
export function isGP(sequence :  number[]){
    const reason = sequence[1] / sequence[0];
    for(let i = 1; i < sequence.length; i++){    
        if((sequence[i] / sequence[i - 1]) !== reason){
            return false
...
export function possibility(percentual : number, total : number = 1){
 ...
 */
export function countdown(callback : Function, time : number, timeTick : number = 1){
